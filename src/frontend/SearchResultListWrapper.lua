...
function SearchResultListWrapper:Initialize(tradingHouseWrapper, searchManager)
    ...
    local list = ZO_SortFilterList:New(CreateProxyControl(tradingHouse.m_searchResultsList))
    list.emptyRow = tradingHouse.noSearchItemsContainer

    local sortHeaderGroup = tradingHouse.m_searchSortHeaders
    ...
    ZO_PlayerInventory_InitSortHeader(nameHeader, SI_TRADING_HOUSE_SORT_TYPE_NAME, TEXT_ALIGN_LEFT, SORT_ORDER_ID.ITEM_NAME_ORDER)
    ...
    local function UpdateResultTimes(rowControl, result)
    ...
    function list:FilterScrollList()
        ...
        if(activeSearch) then
            ...
            local view = tradingHouseWrapper.itemDatabase:GetFilteredView(guildName, filterState)
        ...
        tradingHouse:UpdateItemsLabels(#scrollData)
    end
