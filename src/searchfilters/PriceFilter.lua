...
function PriceFilter:InitializeControls(name, tradingHouse)
    ...
    priceRangeLabel:SetHidden(true)
    self:SetLabel(priceRangeLabel:GetText():sub(0, -2)) -- TODO
...
function PriceFilter:BeforeRebuildSearchResultsPage(tradingHouseWrapper)
...
function PriceFilter:FilterPageResult(index, icon, name, quality, stackCount, sellerName, timeRemaining, purchasePrice)
    return not (purchasePrice < self.min or purchasePrice > self.max)
end
