...
function GuildHistoryHelper:GetOrInitializeGuildState(guildId)
    if not self.state[guildId] then
...
function GuildHistoryHelper:RequestEvents(guildId)
...
function GuildHistoryHelper:RequestData(normalizedTime)
    ...
    for i = 1, GetNumGuilds() do
        ...
        local guildState = self:GetOrInitializeGuildState(guildId)
        if not guildState.initialized or (normalizedTime < guildState.oldest and guildState.hasMore) then
            guildState.initialized = true
            if self:RequestEvents(guildId) then
